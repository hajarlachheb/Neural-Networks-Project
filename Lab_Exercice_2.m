    ...
    for i = 1:length(numEpochsList)
        for j = 1:length(learningRatesList)
            for k = 1:length(mcRatesList)
                ...
                [trainAcc, valAcc, testAcc] = model1(hiddenUnits(hiddenUnitsIdx), trainingRatio(datasetRatiosIdx), validationRatio(datasetRatiosIdx), testRatio(datasetRatiosIdx), X , Y, numEpochsList(i), learningRatesList(j), mcRatesList(k));
...
%Applying the model1 algorithm
fprintf("Begin testing with Model 1 \n")
for i = 1:length(hiddenUnits)
    for j = 1:length(trainingRatio)
        ...
        [trainAcc, valAcc, testAcc] = model1(hiddenUnits(i), trainingRatio(j), validationRatio(j), testRatio(j), X, Y, numEpochs, learningRate, momentum);
